...
Un simple INI loader, c'est pas opti mais ça marche ! ;)
...
public:
	...
	bool Load(const char* filename, bool store_texts = true)
	{
		...
		while ((c_i = fgetc(pFile)) >= EOF)
		{
			...
			if (c_i == -1 || c == '\n')
			{
				if (cur_len >= 3 && seperator_pos != -1 && !skip_cur_line)
				{
					...
					if (name != NULL)
					{
						...
						if (value != NULL)
						{
							...
							{
								name = NULL; // Les pointeurs sont maintenant stockés dans le vecteur, ne pas les libérer ici.
								value = NULL;
							}
				...
				else if (cur_len == 0 || (skip_cur_line && !is_tag))
				{
